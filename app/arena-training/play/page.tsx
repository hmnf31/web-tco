...
export default function PlayBotPage() {
  ...
  }, [ctx.gameOver, ctx.bot.name, ctx.gameResult, ctx.playerColor])
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-white/50">Pilih lawan dari Bot Anggota Divisi TCO Chess atau engine catur</p>
          {ctx.engineReady && (
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="mt-3 flex items-center justify-center gap-2">
              <span className="inline-flex items-center gap-1 rounded-full px-3 py-1 text-[10px] font-medium bg-green-400/10 text-green-400 border border-green-400/20">
                <Cpu className="h-3 w-3" /> Engine Ready
              </span>
            </motion.div>
          )}
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="space-y-5">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => controller.startGame("white", skillLevel)}
                disabled={!ctx.engineReady}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-cyan-500/25 transition-all hover:scale-[1.02] disabled:opacity-40 disabled:cursor-not-allowed">
                <Play className="h-4 w-4" /> Main Putih
              </button>
              <button
                onClick={() => controller.startGame("black", skillLevel)}
                disabled={!ctx.engineReady}
                className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/[0.03] px-6 py-3.5 text-sm font-bold text-white transition-all hover:scale-[1.02] hover:bg-white/[0.05] disabled:opacity-40 disabled:cursor-not-allowed">
